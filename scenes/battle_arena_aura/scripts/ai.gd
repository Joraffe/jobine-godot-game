...
func set_element_names(new_element_names : Array[String]) -> void:
	...
	print('ai.set_element_names called ', new_element_names)
...
func _on_add_elements_animation_finished(instance_id : int) -> void:
	...
	print('ai._on_add_elements_animation_finished')
	print('self.combiner.has_combo() ', self.combiner.has_combo())
...
func _on_remove_elements_animation_finished(instance_id : int) -> void:
	if not self.applicable(instance_id):
		return
	print('_on_remove_elements_animation_finished called')
...
func _on_combo_animation_finished(instance_id : int) -> void:
	...
	self.defer_combo_effects_to_group(self.combiner)

	print('_on_combo_animation_finished')
#	BattleRadio.emit_signal(
#		BattleRadio.COMBO_EFFECT_QUEUED,
#		self.entity_instance_id,
...
func check_for_all_animations_finished() -> void:
	if self.is_removing_finished and self.is_repositioning_finished:
		print('check_for_all_animations_finished both finished')
...
func defer_combo_effects_to_group(combiner_to_defer : Combiner) -> void:
	var group_name : String = self.get_group_name()
	self.emit_combo_effects_deferred_to_group(group_name, combiner_to_defer)
