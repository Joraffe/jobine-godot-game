...
func render_party_swap_members() -> void:
	...
	var top_member_data = swap_data.members[top_position]
	...
	var bottom_member_data = swap_data.members[bottom_position]
...
func instantiate_member(character_data : BattleArenaCharacterData, position : String) -> Node2D:
...
func position_swap_member(instance : Node2D, position : String) -> void:
	var member_area_2d = instance.get_node("Area2D")
	var member_swap_icon = instance.get_node("SwapSprite2D")
	var pos_y : int
	if position == BattleFieldSwapData.TOP:
		pos_y = 100
	elif position == BattleFieldSwapData.BOTTOM:
		pos_y = -100
